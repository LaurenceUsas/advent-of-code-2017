...
import "github.com/LaurenceUsas/advent-of-code-2017/helpers"
import "fmt"
...
//Task05 Solution
func Task05() {
	input := helpers.InputFile("/input/input05.txt")
	maze := helpers.StringArrayToIntArray(input)

	var steps, currentPos, nextPos int
	currentPos = 0
	for {
		if currentPos < 0 || len(maze) <= currentPos {
			fmt.Println(steps)
			return
		}

		nextPos = currentPos + maze[currentPos]
		maze[currentPos]++
		currentPos = nextPos
		steps++
	}
